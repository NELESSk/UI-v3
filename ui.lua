--[[
    Scoot-style UI rebuild - crisp frames / smooth marquee / popup click-away
    Original implementation written from scratch.
    Visual reference: Scoot-Modified / Library.lua
]]
...
-- Repeat Execute: unload the previous library instance first so its
-- input connections do not stay alive, then remove any leftover GUI.
...
local Theme = {
    -- Pure grayscale base: no green tint.
...
local function create(className, props)
    ...
    -- Slight rounding on bordered/control surfaces only.
    -- Decorative 1px accent/glow frames remain untouched.
...
local function textStroke(label, transparency)
    -- Fully disable the fake outline: at small sizes Roblox text becomes
    -- noticeably softer/blurry when TextStroke is visible.
...
-- Always read mouse movement from the same coordinate source.
-- Mixing GuiObject InputBegan.Position with later mouse events can produce
-- a constant vertical offset on some Roblox/executor combinations.
...
-- Clicking anywhere on the normal GUI outside an opened popup closes it.
-- Clicking inside the popup or its source control leaves it open.
connect(UserInputService.InputBegan, function(input)
    ...
    -- Do not let the same physical click that opened a popup immediately
    -- close it again. This was why Target Part looked impossible to open.
    ...
    -- Roblox/executors can expose mouse coordinates with different GUI-inset
    -- conventions. A popup counts as "inside" if EITHER coordinate system
    -- lands inside it. This prevents the first click on Color Picker from
    -- being misread as an outside click.
...
local function makePreviewDraggable(handles, target, accentColor, onMoved)
    ...
    local function beginDrag(input)
        ...
        -- The real GUI is offset-positioned with AnchorPoint 0.
        -- Copy its exact Position/Size into the preview instead of converting
        -- through AbsolutePosition. This guarantees edge-for-edge alignment.
        ...
        -- Exact first frame: no offset above/below the real GUI.
        preview.Position = target.Position
    ...
    connect(UserInputService.InputEnded, function(input)
        ...
        -- Save the permanent resting position immediately. Open/close
        -- animations must never use an intermediate animated position.
        if type(onMoved) == "function" then
            onMoved(destination)
        end

        -- Preview disappears on the release event itself.
        if preview and preview.Parent then
            preview:Destroy()
        end
...
-- Dedicated bottom-right resize.
-- Restored after the drag-preview rewrite accidentally removed this function.
local function makeCornerResizable(target, handle, minWidth, minHeight, linkedSurface, onSizeChanged)
    ...
    connect(handle.InputBegan, function(input)
        ...
        -- Main window is AnchorPoint (0, 0), so these are stable coordinates.
        startPosition = Vector2.new(
            target.Position.X.Offset,
            target.Position.Y.Offset
        )
    ...
    connect(UserInputService.InputChanged, function(input)
        ...
        -- Keep the top-left position fixed.
        -- The resize corner can only expand/shrink toward the bottom-right.
    ...
    connect(UserInputService.InputEnded, function(input)
        ...
        if targetSize then
            -- Snap the final size exactly to the requested integer size.
...
local function attachSmoothMarquee(viewport, textLabel, centerWhenShort)
    ...
    local function refresh(newText)
        ...
        task.defer(function()
            ...
            -- Exact travel distance. The last glyph is fully visible before
            -- the marquee reverses.
            local endX = -(textWidth - available + endPadding)
            local travel = math.abs(endX - startPadding)

            -- Faster than the previous build, but still readable.
...
function Library:SetThemeColor(role, color)
    ...
    elseif role == "InnerTabs" then
        -- "Inner Tabs" controls both section/popup surfaces and controls.
        Theme.Inline = color

        -- Keep controls slightly separated from the section background.
...
function Library:Window(data)
    ...
    local window = setmetatable({
        Name = data.Name or data.Title or "Scoot",
        Logo = data.Logo or "",
    ...
    -- Thin moving edge makes the wipe feel like the GUI is being "eaten"
    -- instead of covered by an empty rectangle.
    ...
    -- The surface stays local to WindowClip.
    -- Moving it to AbsolutePosition here would place it outside its own
    -- ClipsDescendants parent and make the whole GUI disappear.
    main.AnchorPoint = Vector2.new(0, 0)
    main.Position = UDim2.fromOffset(0, 0)
    constrainWindow(windowRoot, 650, 440)

    -- Keep the full-size surface synchronized after initial clamping.
    ...
    -- Every outer edge/corner moves the GUI except the dedicated bottom-right resize corner.
    ...
    -- One clearly dedicated resize corner.
    ...
    -- Small diagonal grip so it is obvious which corner resizes.
    for i = 0, 2 do
        create("Frame", {
            Parent = resizeHandle,
            AnchorPoint = Vector2.new(1, 1),
...
function WindowMethods:SetOpen(state)
    ...
    -- Never read root.Position here: during an interrupted wipe it may be
    -- halfway down the screen. RestPosition is the only permanent position.
    ...
    if state then
        -- OPEN: bottom -> top.
        --
        -- Begin as a 0px-high clipping slice at the bottom edge.
        -- The surface is offset upward inside that slice so it keeps the same
        -- screen-space position while the mask expands upward.
        root.Visible = true
        ...
        -- CLOSE: top -> bottom.
        --
        -- The clipping root moves downward while its height shrinks to zero.
        -- Simultaneously the full GUI surface moves upward inside the root.
        -- Result: the TOP disappears first and the wipe boundary travels down.
        root.Visible = true
        ...
        task.spawn(function()
            ...
            if root and root.Parent then
                ...
                -- Restore to the permanent resting position, never the
                -- intermediate position from an interrupted animation.
                local finalRest = self.RestPosition or Vector2.new(openX, openY)
                root.Position = UDim2.fromOffset(finalRest.X, finalRest.Y)
                root.Size = UDim2.fromOffset(fullSize.X, fullSize.Y)
            end
...
function WindowMethods:Page(data)
    ...
    function page:Turn(state)
        -- Never skip the inactive visual reset. When switching tabs quickly,
        -- another page may already have set Active=false before this page gets
        -- its own Turn(false) call. Skipping here left several tabs highlighted.
        ...
        if state then
            -- Kill old page content instantly so pages can never overlap.
            -- Do NOT mutate other.Active here: each tab receives Turn(false)
            -- from the selection loop below, which also resets its visuals.
            ...
            -- First-version selected-tab visuals.
            tween(self.Button, {
            ...
            -- Hide immediately so rapid tab clicking cannot overlap pages.
    ...
    connect(button.MouseButton1Down, function()
        ...
        -- Reset ALL tabs first, then enable exactly the clicked one.
        -- This makes "one selected tab only" deterministic even when spam-clicking.
...
function SectionMethods:Toggle(data)
    ...
    -- Only the square itself toggles the option.
    -- Clicking the text/empty row area does nothing.
...
function SectionMethods:Keybind(data)
    ...
    -- Left-side name has its own clipped viewport so a long option name
    -- never pushes underneath the bind box.
    ...
    -- The box itself is only a container now. The key text and mode tag
    -- are physically separated, so RIGHTSHIFT can never draw behind "T".
    ...
    -- Slow marquee helper. It only runs when text is wider than the
    -- available clipped viewport; short text remains perfectly still.
...
function SectionMethods:Colorpicker(data)
    ...
    connect(row.MouseButton1Down, function()
        ...
        Library:SetOpenFrameCloser(
            ...
            row,
            function(point)
                -- MouseEnter/MouseLeave is the primary guard because it is not
                -- affected by GUI inset coordinate differences. Rectangle
                -- checking remains as a fallback.
                return pickerMouseInside or pointInsideGui(popup, point)
            end
        )

        -- The drag strip itself is the clickable control.
        -- No child TextLabel sits on top of it, so the FIRST press always
        -- reaches InputBegan and immediately starts dragging.
        ...
        popupConnect(pickerDrag.InputBegan, function(input)
            ...
            -- Delta-only dragging: no conversion from mouse coordinates to
            -- GUI coordinates, so the picker never snaps/recenters.
        ...
        -- Visible circular point showing the currently selected S/V position.
...
function Library:Notify(data)
    ...
    -- Expand under the actual text instead of scrolling it.
    local CARD_W = clamp(
...
-- Export the reusable library when loaded through game:HttpGet/loadstring.
if typeof(getgenv) == "function" then
    local ok, env = pcall(getgenv)
    if ok and type(env) == "table" then
